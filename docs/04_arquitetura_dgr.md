...
| **Viabilização de oportunidade** | `src/dg/viabilizacao.ts`, `src/services/viabilizacaoService.ts` | Quando não dá para atender, o DG explica o porquê, oferece opções A/B/C e **planos** (combinações), filtra o que **não cabe no PRAZO** do cliente (inviável), e recomenda conforme o **PERFIL DE RISCO** (conservador/equilibrado/arrojado, inferido do comportamento) — atender, renegociar prazo ou recusar. Projeta faturamento/custo/lucro-prejuízo e valor estratégico. **Decisão fica com o dono** e alimenta a base anonimizada (consentimento + N≥X). Gating: dg_consultor full (Pro+). |
