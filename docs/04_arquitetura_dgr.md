...
| **Viabilização de oportunidade** | `src/dg/viabilizacao.ts`, `src/services/viabilizacaoService.ts` | Quando não dá para atender, o DG explica o porquê, oferece opções A/B/C, recomenda com fatores, projeta faturamento/custo/lucro-prejuízo e valor estratégico — **decisão fica com o dono**. A decisão alimenta a base anonimizada (consentimento + N≥X). Gating: dg_consultor full (Pro+). |
